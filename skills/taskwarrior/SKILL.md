...
- Tasks you touch through `taskwarrior_run` show up in the thread's Tasks tab (Recent, with an "agent" badge). Name tasks by id or uuid in the command (`["12", "done"]`) rather than relying on listings; report output like `list` is not recorded.
